...
export const AdvantagesForm = () => {
  ...
  const deleteAdvantage = (index: number) => {
    const newAdvantages = [...formik.values.advantages];
  ...
  return (
    <div className={s.advantage}>
      {formik.values.advantages.map((advantage: string, index: number) => (
        <div className={s.item} key={index}>
          <TextField type="text" id={`field-advatages-${index + 1}`}
            name={`advantages[${index}]`} placeholder="Преимущества" />
          <svg id={`button-remove-${index + 1}`} onClick={() => deleteAdvantage(index)} className={s.icon} width="16" height="17" viewBox="0 0 16 17" fill="#CCC" xmlns="http://www.w3.org/2000/svg">
      ...
      {formik.errors.advantages && (
        <div className={s.error}>{formik.errors.advantages}</div>
      )}
    </div>
  );
}
